            ...
            <div class="w-full lg:w-3/4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    @if($culinaries->isEmpty())
                        <p class="text-center text-3xl">Tidak ada Kuliner</p>
                    @else
                        @foreach($culinaries as $culinary)
                            <div class="border rounded-lg overflow-hidden shadow-lg product-item flex flex-col">
                                <img alt="{{$culinary->business_name}}" class="w-full h-48 object-cover mb-4" height="200" src="{{asset('storage/img/'. $culinary->image)}}" width="200"/>
                                <div class="p-4 flex-1">
                                    ...
                                    <p class="bg-yellow-500 text-black py-2 px-4 rounded-lg w-56"><a href="{{ route('culinary.culinaryDetail', $culinary->id)}}">LIHAT DETAIL PRODUK</a></p>
                                </div>
                            </div>
                        @endforeach
                    @endif
